...
namespace BetterAfrica.Benefits.Entities
{
    public class AulPolicy : BContract
    {
        ...
        // Although the Person has a member type in Better Africa, each policy
        ...
        public IEnumerable<CPerson> GetDependencies(EDependencyType type)
        {
        ...
        public CPerson Principal
        {
            get
            {
                var dependants = Dependancies.Where(p => p.Type == EDependencyType.Principal).ToList();
        ...
        public CPerson Spouse
        {
            get
            {
                var dependants = Dependancies.Where(p => p.Type == EDependencyType.Spouse).ToList();
                if (dependants.Count == 1) return dependants[0].Person;
        ...
        public string SpouseError
        {
            get
            {
                var spouses = GetDependencies(EDependencyType.Spouse).ToList();
                if (spouses.Count() > 1)
        ...
        public IEnumerable<CPerson> Children => GetDependencies(EDependencyType.Child);

        /// <summary>Persons covered by the policy as family.</summary>
        public IEnumerable<CPerson> Family => GetDependencies(EDependencyType.Family);
            ...
            foreach (var dependancy in Dependancies)
            {
                ...
                if (InceptionDate != null)
                {
                    ...
                    if (person.AgeInYearsAsAt(Clock.Now) >= maxAgeInYears)
                        Errors.Add(nameof(CMember.Dependencies), $"{type} '{person.Name}' cannot be older than {maxAgeInYears} for policy {Number}.");
                }
            }
        }

        /// <summary>Add a person of a specific type to be covered.</summary>
        public AulPolicy WithDependency(CPerson person, EDependencyType type)
        {
